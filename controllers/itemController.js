...
const itemController = {
  ...
  getItemById: asyncHandler(async (req, res) => {
    ...
    const image64 = item.image ? item.image.data.toString('base64') : null;

    console.log(image64)
    res.render("item-detail", { title: "Item ", item: item, category: category, image: image64 });
  }),
  ...
  createItem: asyncHandler(async (req, res) => {
    ...
    if (req.file) {
      //base64
      newItem.image.data = req.file.path;
